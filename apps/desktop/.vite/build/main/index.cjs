...
function getDbFilePath() {
  ...
  console.log("[DB FILE]", electron.app.getPath("userData"));
  return path$1.join(dir, "app.sqlite");
}
let _db = null;
let _sqlite = null;
function getDb() {
...
function registerDbIpc() {
  electron.ipcMain.handle(IPC.DB_SETTINGS_GET, async (_evt, key) => {
    const db = getDb();
    const rows = await db.select().from(settings).where(eq(settings.key, key)).limit(1);
    return rows[0]?.value ?? null;
  });
  electron.ipcMain.handle(IPC.DB_SETTINGS_SET, async (_evt, key, value) => {
    const db = getDb();
  ...
  electron.ipcMain.handle(IPC.DB_EVENTLOG_APPEND, async (_evt, type, payloadJson) => {
    const db = getDb();
  ...
  electron.ipcMain.handle(IPC.DB_EVENTLOG_LIST, async (_evt, limit = 100) => {
    const db = getDb();
    return db.select().from(eventLog).limit(limit);
  });
}
function registerIpcHandlers(getMainWindow) {
  electron.ipcMain.handle(IPC.PING, async () => ({ ok: true, ts: Date.now() }));
  electron.ipcMain.handle(IPC.WIN_CLOSE, async () => {
    getMainWindow()?.close();
  });
  electron.ipcMain.handle(IPC.WIN_MINIMIZE, async () => {
    getMainWindow()?.minimize();
  });
  electron.ipcMain.handle(IPC.WIN_MAXIMIZE, async () => {
    getMainWindow()?.maximize();
  });
  electron.ipcMain.handle(IPC.WIN_TOGGLE_MAXIMIZE, async () => {
...
function runMigrations() {
  ...
  const sqlite = new Client(dbFile);
  ...
  `);
  const migrationsDir = electron.app.isPackaged ? path$1.join(process.resourcesPath, "drizzle") : path$1.join(process.cwd(), "drizzle");
  ...
  sqlite.transaction(() => {
    for (const f of files) {
      ...
      if (already) continue;
      const sql2 = fs.readFileSync(path$1.join(migrationsDir, f), "utf-8");
      sqlite.exec(sql2);
